...
4. **The ground** (`groundShader`), from the photo and its aux map:
   ...
   - **The flock's reflection** darkens the water. Mirroring in level water flips a point about the horizon on screen, so each frame the flock splats each parcel's ink at its flipped position into a 384×128 texture (`Flock.reflection`), saturating as d / (1 + d). It covers from the horizon down to 1.4 horizons below the screen's bottom, because on the wavy sea the shader reads it the way it reads the sky: five taps from the mirror image up toward the horizon (to 3.3 × `rough`, weighted e^−0.7k toward the mirror), so a flock whose mirror image is off the screen still leaves a faint streak in the sea under it. On the calm pond `rough` is tiny and it stays a mirror. Only rows the flock touched this frame or last are converted and uploaded, and nothing is uploaded while the flock is too high to reflect, which is most of the time.
...
- "I am not seeing much of any reflection on the pier one ... a flock came really close to the screen and I saw no reflections at all." On calm water that's correct: a close, high flock's mirror image is below the frame, which shows only about 12° down. But the pier's reflection of the flock ignored the waves that its reflection of the sky already used; now it streaks up toward the horizon (up to about 17 levels darker under a close flock). He also wondered about shadows: at sunset, with the Sun low and ahead, a flock's shadow falls hundreds of metres behind the viewer, and after sunset there is none, so none are drawn.
