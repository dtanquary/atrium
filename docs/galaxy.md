...
5. **Light, from a bright disc the arms brighten.** In Hubble images the space between the arms isn't dark: the whole disc is a smooth glow, cream toward the middle and blue-grey outward. The arms are only two to three times brighter than the disc around them. Drawing arms on a dark disc is what made the first version look crisp and CG.
   - disc: `exp(-rt/0.4)`, fading out between 0.85 and 1.4 with no edge, since the photo drops off faster from about three quarters of the way out. `rt` gives the disc a thickness of 0.15, so steep tilts fade to a soft ellipse instead of a sharp one, and it's also what the branch at 1.6 tests.
   ...
   - texture: photos have fine texture everywhere, about ±25% at 1–10 px, and a smooth disc reads as airbrushed. So `tex = 0.7 + 0.6·(0.45·lumps + 0.35·noise(g·110) + 0.2·noise(g·260))` (octaves turned apart) multiplies the disc and arm light.
   - light: `tint·disc·(0.84·tex + 2.5·arm·tex)·√los` (the arm weight went from 2.1 to 2.5, since texture lowers the arm contrast). `tint` shades from bulge cream to disc white outward, and toward the young-star blue in the arms and the outskirts (`0.6·smoothstep(0.4, 1.1, r)`, which gives the photo's teal-grey at 0.7–0.8 of the radius). `los = min(1/cos i, 4.5)` is the path through the disc, so tilted discs look brighter.
...
7. **Resolved stars (`discStar`):** one candidate per cell of `g`, on a grid turned by `turn`. The layers:
   - the fine grain, which follows the light: two layers, 2.8 pt and 3.9 pt cells, on grids turned 37° apart so their patterns never line up (as in the background sky). Their density is `crowd` (up to 1 on the arms, `disc·0.6` elsewhere), bunched into clusters along the arms by the star-cloud noise (`0.45 + 1.1·clump`), and kept up to 80% and 90%. Each star is as bright as the disc around it and brighter on the arms (`min(disc·(1.5 + 3·arm), 0.3)`).
   - every star is a 1.5 px pinpoint (`exp(-d²·(4.5 - 2.5·mag))`, with `mag = h^2.5`), like the photo's 1.3 px. It can sit anywhere in 95% of its cell: jitter over a whole cell cancels the grid's own spacing out of the pattern, while ±40% kept about 5% of it, which showed in dense arms.
   - 11 pt cells, on a grid turned 60°, for bright blue giants just past the crests
...
8. **H II regions (`knot`):** 16 pt cells, kept along `hii` on the arm's inner edge, only where a slow noise (`groups`) allows, and only inside r ≈ 1.2, so they come in chains and complexes. Each is 0.8–3.2 pt, small enough to stay inside its cell; at up to 4.5 pt they overflowed it and showed as clipped half-moons. Small knots can sit almost anywhere in their cell and big ones nearer its middle (`0.95 - 3·size/16`, from 30% to 90%); a fixed ±15% made the chains along the arms evenly spaced. The pink hydrogen glow (`u_knots`) surrounds a blue-white cluster core, dimmer toward the outskirts. Part of the pink (`vivid`, ×0.7) is added after the stretch, the way Hubble images keep H-alpha saturated; the rest (×1.5) goes through it. They're kept up to 90% where `groups` (a noise at `g·9`, from 0.3) allows, ×12. They're round on screen, like the stars.
9. **Bulge:** a Sérsic n=2 profile (`20·exp(-3.67√rb)` at 1.7× the kind's bulge size) plus a nucleus, measured in `e` with an axis ratio of `mix(cos i, 1, 0.6)` (bulges are rounder than discs). The disc cuts through its middle. `behind` is the share of bulge light behind the dust: half face-on, more on the near side of a tilted galaxy. That gives the Andromeda-style dust silhouette across the bulge.
9b. **Steep tilts (Andromeda):** tilted past about 60°, the disc is squashed so hard that arm edges and dust lanes turn razor-thin on screen, where real M31 is a soft glow with smeared arms. `steep = smoothstep(0.5, 0.25, cos i)` (so only Andromeda) does four things:
    - lowers the arm, young-star and H II exponents to 0.4×
    - lowers the lane exponent to 0.7× and the filament web to 0.4×
    - fades the arms out by r ≈ 1.25, so they don't make a bright rim at the disc's edge
    - adds a thick-disc haze, `exp(-rh/0.45)·0.12`, where `rh` uses a rounder axis ratio, `mix(cos i, 1, 0.3)`

    Steep galaxies draw within `rh < 1.6` rather than `rt < 1.6`, so the haze fades out instead of being cut off.
...
11. **Composite: a Hubble-style stretch.** Brightness gets `asinh(20·lum)/asinh(240)`, the log-like curve used to process Hubble and ESO images. K = 20 fits M51's radial profile within 5% from 0.1 to 0.7 of its radius; K = 6 was off by 15%. It lifts faint light a lot, so the arm weight went to 3.5 to keep arm contrast, and the bulge is broader (1.7× its size) and brighter (×20) and drawn outside the branch, so its faint outskirts never meet the branch's edge. This stronger stretch is on trial (Dave took all the research); it flattens NGC 1300 and M51 somewhat, so revert its own commit if the contrast is missed. It lifts the faint outer disc so the galaxy fades gradually instead of stopping, and holds the core. It's applied to brightness and not per channel, so colours keep their saturation; the brightest parts pale toward white, as on a real sensor. `col = sky·absorb + stretched light`. The sky behind is described in the next step. `brightStar` foreground stars go on top, untouched by the galaxy's dust, since they're in our own galaxy. Then grain: `(h.x + h.y − 1)·0.02·√stretched` from `hash42` of the pixel, so it's strongest where the galaxy is brightest, like shot noise, and the empty sky stays clean black. The grain is fixed to the screen, like a sensor's, while the galaxy turns beneath it. The sky itself is a Hubble frame's dark grey, (8, 9, 11)/255 (the reference frames sit at 4–27), with faint grain (`0.008`) across it. This is on trial: Dave took all the research, but Nebula keeps a black sky, and grey can look washed out on a desktop, so revert its own commit if it does. Last, a `/128` dither.
...
- Stars: 2.8 pt and 3.9 pt grain layers kept up to 80% and 90% (×`min(disc·(1.5 + 3·arm), 0.3)`), and 11 pt giants up to 30% (×0.5). H II: 16 pt cells, up to 90% where grouped, inside r ≈ 1.2, ×2.2.
...
- **Grids of stars:** one star per cell shows the grid once cells are nearly full and stars can't roam their whole cell, even with a hash that never repeats. Dave asked for the galaxy's own stars to be checked, like the sky. With only the fine star layer drawn, white on black, a Fourier transform of a 512 px patch scored the grid's peak against a baseline of the same stars scattered at random:
  - one layer, ±40% jitter: 1,700–1,900 on Pinwheel (random: 300–350) and 650–930 on Andromeda (random: 500–550)
  - now: about 260 and 210, below the random baseline, so there's no lattice left

  To repeat the test, draw a single layer in the shader and measure it; the scripts are `debug.py` and `fft.py` in the session's scratch folder, not the repo.
...
- "Double check… that stars we spawn inside a galaxy don't have the same patterning issue." They did: a lattice 5–6× stronger than random in Pinwheel's dense arms. Two turned, clustered, fully jittered layers fixed it (see Gotchas).
...
- "Andromeda is the only one that seems a bit off… needs like a bit more glow or edge blurring on the spiral arms." That became the `steep` softening and thick-disc haze. He loves the companions: "that background galaxy in view too, just like in real life."
