...
- **`build()`** throws everything away and rebuilds it for the current conditions: the sky, the ground, rain or snow, and lightning. It only runs when the weather changes, and then `redraw()` crossfades over 4 s from a snapshot of how the scene looked (`SKView.texture(from:)`). Day and night need no rebuild: the sky bakes every minute and eases between bakes.
  ...
  - **Ground:** a photo of Fort Ord National Monument (BLM California, public domain, 7379 px wide, shot on an overcast May morning, so there are no hard shadows to fight the real Sun) with the sky cut out, as `weather-ground.heic` (4096×1485 with alpha, 1.4 MB). `weather-ground-aux.png` (2048×743) holds its distance in red and its trees in green. Both are decoded once, statically.
    ...
    - **Cloud shadows** (`castShadows`, fair days only, the Sun above about 6°): four lumpy ellipses on the ground, each about its cloud's size, its edges broken up by the billow noise, fading in and out with the cloud's life. Each ground pixel knows roughly where it is in km from the depth map (`(32^d − 1)/3.1` away, across by the view's angle), so a shadow lies on the terrain and a ridge in front hides the part behind. Inside a shadow only skylight is left (`u_shade`: `0.35 + 0.65·skylight/all light`, darker and a little bluer), and the haze on top fades far shadows away.
      - Two are the nearest visible clouds' shadows, moved away from the Sun by the cloud's height over the tangent of its elevation, so they slide and stretch with the Sun. Those clouds are 3.5 km off and more, so these land on the middle and far ridges.
      - Two come from clouds overhead and behind, out of the frame (`overhead`), drifting with the same wind and living the same lives, placed 0.05–2.5 km away, more of them near. Most of the ground in view is within a kilometre or two, and in real life the shadows sweeping over the near hills come from clouds above you. Without them the shadows only ever touched the far ridges.
  ...
  - **Cumulus are photos** (`addPhotoClouds`): nine real clouds (eight cumulus and a tower) cut out of CC0 Poly Haven sky panoramas and a CC0 Commons photo by a research agent (keying out the sky, decontaminating the edges, `weather-cloud-*.heic`, 0.5 MB in all). Procedural cumulus came out as flat smears; the photos have real puffs and wisps. Five more cuts were dropped because their clouds ran into the photo's frame, and the straight edge showed at sunset even feathered.
    - **How many:** 6 + 14·cover when partly cloudy, 1–7 on a mainly clear day with some cover, and one in seven a far tower (cumulus congestus). Overcast, rain, snow and storms use the deck instead. With the procedural layer gone on fair days, fewer looked like a nearly empty sky.
    - **Where:** each at a real place, 3.5–34 km away (more far than near, `cloudDistance`) with its base 1.3–1.6 km up, so near ones are big and high on the screen, far ones small and low, and the ridge hides the farthest. Towers stand 26–40 km off with lower bases, so the ridge hides the bottom of their photos. Width is the cloud's real size (1–5 km) over its distance.
    - **Drift:** every frame with the real wind at 1.3× the 10 m speed, times the Cloud speed setting. At real speed and 15 km/h a cloud 10 km off took about 50 minutes to cross the view: true, but it looked still. One that drifts out of view comes back in, already formed, on the upwind side, as a different cloud.
    - **Life:** each forms, grows about 12% and dissolves over 15–30 minutes, as real fair-weather cumulus do, divided by the Cloud speed but never under 3 minutes; then it forms again somewhere else in view as a different cloud. Forming and dissolving raise a clip on the cloud's thickness (`a_life`): the edges go first and the thick core last, as real cumulus evaporate. A plain fade looked like a slide dissolve, and clipping raw alpha broke clouds into scraps and holes, since the photos have thin patches inside; the thickness is the alpha blurred by about 3% of the width (a finding of the billowing research). `packedCloud` repacks each photo at load, since the shader only needs its brightness: red and blue its luminance, green its thickness. The first clouds start part-way through their lives, so they don't all form together.
...
## Settings
- **Weather** (`weather.lock`): "Live where you are" (the default), or one of the eight kinds to lock it to: Clear, Partly cloudy, Overcast, Fog, Drizzle, Rain, Snow, Thunderstorm. A locked kind still follows the real time of day, Sun and Moon. Each stands in as one code (0, 2, 3, 45, 53, 63, 73, 95) with a 15 km/h wind blowing across the view from left to right and 20° away (toward the viewer the clouds only grew, which barely read as motion), 12 cm of snow on the ground for Snow, and 400 m visibility for Fog. The scene keeps polling underneath, so going back to Live shows the latest report at once. Dave asked for it on 2026-09-25: "add some options to settings to lock to various weather conditions in addition to using local".
- **Live status:** under the Weather menu, while it's on Live, a line says what the last report was and when it came, e.g. "Live: Clear (WMO 0) · 14% cloud · wind 10 km/h from E · 42.17°N 88.42°W · updated 8:44 PM" (`Conditions.summary` plus the place and time). The scene writes it to UserDefaults (`weather.status`) on each fetch, from any display; a failed fetch says "Couldn't reach Open-Meteo at …" and keeps the last good report. It only updates while Weather is on the desktop, since polling stops otherwise, so the time shows how fresh it is. Dave asked for it as a confirmation of the live setting.
- **Cloud speed** (`weather.cloudSpeed`, 1–15×, default 6×): how much faster than the real wind the clouds drift. At real speed a near cloud took about 25 minutes to cross the view, and Dave asked whether they moved at all; at 6× the nearest glide at about 5 pt/s. The deck and cirrus drift by an offset accumulated on the CPU (`u_drift`, wrapped at 200 km where the noise repeats; `u_cirrusDrift` in the cirrus noise's own units), so changing the speed never makes them jump, and `u_time`'s size can't cost them precision.
...
## Tuning constants
- **Clouds:** `Conditions.clouds`, per kind. The deck drifts at `wind/3600·0.6` km/s, the photo cumulus at `wind/3600·1.3`, both times the Cloud speed.
...
- **Every state:** `SNAPSHOT_DEFAULTS="weather.lock=7,weather.previewTime=1,weather.previewHour=22" SNAPSHOT_SCENE=Weather swift test` renders a snowy night. Add `weather.cloudSpeed=1` for real-speed drift. The lock is an index: 0 live, 1 clear, 2 partly cloudy, 3 overcast, 4 fog, 5 drizzle, 6 rain, 7 snow, 8 storm. For exact codes, cloud cover or wind, pass `conditions:` to `WeatherScene(size:conditions:)` from a test.
