...
2. **Three curtains** (a loop, `i = 0..2`, each further back and dimmer by `1 - 0.25 i`).
   - **Lower edge:** `edge = 0.5 + 0.1i + 0.045·fold + 0.08·(noise − 0.5)`, so even the lowest fold stays clear of the summits. The fold is two slow sines (0.04 and 0.03 rad/s) and the noise drifts at 0.01, so the curtains fold and ripple without looping.
   - **Light:**
...
3. **Soft clip.** `1 − exp(−aurora × 0.9)`, so overlapping curtains don't blow out. It fades above `uv.y` 0.72–1.12.
