...
- **Files:** `Sources/Atrium/Aurora.swift` (the palettes, the arc texture, the sky shader, `AuroraCurtains` and the ground), plus `Resources/aurora-ground.heic` and `Resources/aurora-ground-aux.png`. The shaders use the `shaderCommon` helpers `noise`, `hash21`, `starField(pts, cell, density, t)` and `grade`.
...
- **Kind:** a full-screen SKShader for the sky under a photo sprite with its own shader for the ground. Swift side: the palette, a baked arc texture on each load, a clock (`u_phase`, advanced by an SKAction in scene time), and `AuroraCurtains`, which holds the curtains, replaces them one at a time, and re-measures their light on the land each second.
...
2. **Curtains** (up to four, `u_dist`, `u_angle`, `u_lum`, `u_low`, `u_thick`, `u_offset`, from `AuroraCurtains`): each is a vertical sheet over a line on the ground `dist` km away, its normal `angle` from north, leaning 11° toward us along the field. One main arc at 220–330 km, a second (45% of loads) and a third (20%) at 180–380 km, all near the display's heading. Those distances keep every lower edge above the summits. Each reads its own stretch of the arc texture (`u_offset`), so a new curtain brings new folds and rays.
...
4. **Ground.** A photo of the Tetons from Teton Point in winter (NPS photo by A. Falgoust, public domain, 6000×4000, a soft day under thin high cloud): snow flats, a dark band of spruce, then the jagged range. It's drawn as its own sprite over the sky, full width, with its highest summit at 0.38 of the screen height (the skyline runs 0.26–0.38) and its bottom cropped by the screen.
   ...
   - **Lit by the aurora** (Dave: "is it possible to have the aurora cast a realistic light on the mountain range below"): `2·tex^2.2` is the albedo, times `u_star` (starlight and airglow, bluish, 0.02) plus `u_aurora` (the palette's body and upper colours at unit brightness, 0.016) × `pool` × `open`.
     - `pool` is how bright the arc is above that part of the screen: `AuroraCurtains.pool()` runs the shader's own geometry on the CPU (the same arc texture bytes, folds, path factor and patches, at the curtains' lower edges) for 8 strips across the screen, once a second, into `u_poolA`/`u_poolB`, which the shader interpolates. Snow under a bright stretch comes out about 40% brighter and a little greener than under a dim one.
...
- **Motion, rearranging:** `aurora.rearrange`, "Slowly rearrange the curtains" (on), and `aurora.rearrangeMinutes`, "Every", 1–30 min (4). Dave asked for it on 2026-09-25, "so that if someone has that wallpaper set they will get a variety of aurora over time". Each time, `AuroraCurtains.rearrange()` retires one curtain and, unless the sky is to hold fewer (1, 2 or 3, weighted 2:2:1), brings a new one into a free slot: the main arc if no other bright one stays. They cross-fade at 1/90 of full brightness per second, so the sky is never empty and never cuts, and the display's heading wanders ±0.15 rad (within ±0.4) each time, so over 10–20 minutes it's a different aurora. It runs on phase time, so it follows Speed (and stops at 0×).
- **Colors, fading:** `aurora.fade`, "Fade to a new color automatically" (off), and `aurora.fadeMinutes`, "Every", 1–60 min (10), shown under the swatches only while the palette is Random, as Lava Lamp's cycling is. Dave asked for it on 2026-09-25. Every so often the four colours, the crown strength and the ground's aurora light ease to another palette over 90 s (smoothstep, in linear light, per `AuroraColours.mixed`). The clock counts from the start of each fade, so at 1 minute the fades run back to back. These knobs, and Speed, aren't shader uniforms (only the Look sliders are) (they're read once a second from UserDefaults), which keeps the sky shader under Metal's limit of about 30 uniforms.
...
- 2026-09-25, the aurora itself: the techniques agent's perspective-sheet prototype, tuned against the reference agent's measurements, behind a Compare switch for Dave to choose. He chose it: "this new version is so much better", so the old curtains and the switch were removed (they're in git history before that commit, with their colours). He also asked for the aurora to "cast a realistic light on the mountain range below" (the ground section above).
