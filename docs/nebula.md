...
- **Files:** `Sources/Wallpaper/Shaders.swift`: the `nebulaPalettes` (file scope) and `nebula(size:)`, which includes its shader source and `brightStar`. `hash21`, `noise`, `fbm` and `starField` come from `shaderCommon` in the same file.
