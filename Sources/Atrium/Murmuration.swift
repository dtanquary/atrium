...
let murmurationKnobs = [lightKnob, groundKnob, falconKnob]
...
/// it folds into dark bands and opens into pale sheets as it wheels.
final class Murmuration: SKScene {
    private var flock: Flock?
    private var lastTime: TimeInterval?
    private var built = (evening: -1.0, ground: -1.0)
    ...
    /// The light and the ground rebuild the scene; the falcon switch is read by the flock as it flies.
...
private final class Flock: SKNode {
    ...
    private func simulate(_ dt: Float) {
        ...
        nextFalcon -= dt
        if nextFalcon < 0 && falconKnob.value > 0.5 {
